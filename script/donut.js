(function (d3) {
    ...
    donut.prototype.redraw = function (callback) {
        ...
        $(p.parent).html('');
        p.minVal = Math.min(p.width, p.height);
        p.radius = (p.minVal * 2 / 3) - (p.minVal * 1 / 7);
        if ((p.width * 2 / 3) <= p.height && p.minVal / 2 < 300) {
            p.radius = p.minVal * 0.4;
        }
        ...
        .outerRadius(p.radius * 0.89)
        .innerRadius(p.radius * 0.55);
        p.outerArc = d3.svg.arc()
        //.innerRadius(p.radius * 0.9)
        //.outerRadius(p.radius * 0.9)
        .innerRadius(p.radius * 0.89)
        .outerRadius(p.radius * 0.89);
        ...
        //
        p.slice.append('title').text(function (d) {
            return [d.data.label, d.data.value].join(' : ')
        });
        p.text[0].forEach(function (el) {
            var d = d3.select(el).data()[0];
            var txt = d.data.label;
            ...
            var z = setInterval(function () {
                if (el.getAttribute('transform')) {
                    clearInterval(z);
                    z = null;
                    var a = el.getAttribute('transform').replace(/translate|\)|\(/g, '').split(',').map(function (v) {
                        return parseFloat(v)
                    });
                    var r = a[0];
                    var l = Math.floor((p.width / 2) - r) - 1;
                    if (width > l) {
                        for (var i = txt.length; i > 0; i--) {
                            if (el.getSubStringLength(0, i) <= l) {
                                el.textContent = txt.substring(0, i - 3) + "...";
                                return;
                            }
                        }
                    }
